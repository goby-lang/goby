...
func TestConcurrentArrayStarOperatorFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2]) * nil`, "TypeError: Expect argument to be Integer. got: Null", 3, 3},
	}
...
func TestConcurrentArrayPlusOperatorFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2]) + true`, "TypeError: Expect argument to be Array. got: Boolean", 3, 3},
	}
...
func TestConcurrentArrayAnyMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([]).any?`, "InternalError: Can't yield without a block", 3, 3},
	}
...
func TestConcurrentArrayAtMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).at`, "ArgumentError: Expect 1 arguments. got=0", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).at(2, 3)`, "ArgumentError: Expect 1 arguments. got=2", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).at(true)`, "TypeError: Expect argument to be Integer. got: Boolean", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).at(1..3)`, "TypeError: Expect argument to be Integer. got: Range", 3, 3},
	}
...
func TestConcurrentArrayClearMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['Maxwell', 'Alexius']).clear(123)`, "ArgumentError: Expect 0 argument. got=1", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['Taipei', 101]).clear(1, 0, 1)`, "ArgumentError: Expect 0 argument. got=3", 3, 3},
	}
...
func TestConcurrentArrayConcatMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.concat(3)
		`, "TypeError: Expect argument to be Array. got: Integer", 4, 4},
		{`
		require 'concurrent/array'
		a = Concurrent::Array.new([])
		a.concat("a")
		`, "TypeError: Expect argument to be Array. got: String", 4, 4},
	}
...
func TestConcurrentArrayCountMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.count(3, 3)
		`, "ArgumentError: Expect 1 argument, got=2", 4, 4},
	}
...
func TestConcurrentArrayDeleteAtMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).delete_at`, "ArgumentError: Expect 1 argument. got=0", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).delete_at(2, 3)`, "ArgumentError: Expect 1 argument. got=2", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).delete_at(true)`, "TypeError: Expect argument to be Integer. got: Boolean", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).delete_at(1..3)`, "TypeError: Expect argument to be Integer. got: Range", 3, 3},
	}
...
func TestConcurrentArrayEachMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['M', 'A', 'X', 'W', 'E', 'L', 'L']).each`, "InternalError: Can't yield without a block", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['T', 'A', 'I', 'P', 'E', 'I']).each(101) do |char|
			puts char
		end
		`, "ArgumentError: Expect 0 argument. got=1", 3, 3},
	}
...
func TestConcurrentArrayEachIndexMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['M', 'A', 'X', 'W', 'E', 'L', 'L']).each_index`, "InternalError: Can't yield without a block", 3, 3},
		{`
		require 'concurrent/array'
...
func TestConcurrentArrayEmptyMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).empty?(123)`, "ArgumentError: Expect 0 argument. got=1", 3, 3},
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['T', 'A', 'I', 'P', 'E', 'I']).empty?(1, 0, 1)`, "ArgumentError: Expect 0 argument. got=3", 3, 3},
	}
...
func TestConcurrentArrayFirstMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.first("a")
		`, "TypeError: Expect argument to be Integer. got: String", 4, 4},
		{`
		require 'concurrent/array'
		a = Concurrent::Array.new([1, 2])
		a.first(1, 2, 3)
		`, "ArgumentError: Expect 0..1 argument. got=3", 4, 4},
		{`
		require 'concurrent/array'
		a = Concurrent::Array.new([1, 2])
		a.first(-1)
		`, "ArgumentError: Expect argument to be positive value. got=-1", 4, 4},
	}
...
func TestConcurrentArrayFlattenMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.flatten(1)
		`, "ArgumentError: Expect 0 argument. got=1", 4, 4},
	}
...
func TestConcurrentArrayJoinMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.join(",", "-")
		`, "ArgumentError: Expect 0 or 1 argument. got=2", 4, 4},
		{`
		require 'concurrent/array'
		a = Concurrent::Array.new([1, 2])
		a.join(1)
		`, "TypeError: Expect argument to be String. got: Integer", 4, 4},
	}
...
func TestConcurrentArrayLengthMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new([1, 2, 3]).length(10)`, "ArgumentError: Expect 0 argument. got=1", 3, 3},
	}
...
func TestConcurrentArrayReduceMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.reduce(1)
		`, "InternalError: Can't yield without a block", 4, 4},
		{`
		require 'concurrent/array'
		a = Concurrent::Array.new([1, 2])
...
func TestConcurrentArrayReverseEachMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		{`
		require 'concurrent/array'
		Concurrent::Array.new(['M', 'A']).reverse_each`, "InternalError: Can't yield without a block", 3, 3},
		{`
		require 'concurrent/array'
...
func TestConcurrentArrayRotateMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		require 'concurrent/array'
		a = Concurrent::Array.new([1, 2])
		a.rotate(1, 2, 3)`, "ArgumentError: Expect 0..1 argument. got=3", 4, 4},
	}
...
func TestConcurrentArrayShiftMethodFail(t *testing.T) {
	testsFail := []errorTestCase{
		...
		a = Concurrent::Array.new([1, 2])
		a.shift(3, 3, 4, 5)
		`, "ArgumentError: Expect 0 argument. got=4", 4, 4},
	}
