	...
	return []*BuiltinMethodObject{
		{
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					if len(args) != 2 {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, errors.WrongNumberOfArgumentFormat, 2, len(args))
					}

					driverName, ok := args[0].(*StringObject)

					if !ok {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, "Expect database's driver name to be a String object. got: %s", args[0].Class().Name)
					}

					dataSource, ok := args[1].(*StringObject)

					if !ok {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, "Expect database's data source to be a String object. got: %s", args[1].Class().Name)
					}
	...
	return []*BuiltinMethodObject{
		{
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					...
					if err != nil {
						if err != nil {
							return t.vm.initErrorObject(errors.InternalError, instruction.sourceLine, "Error happens when closing DB connection: %s", err.Error())
						}
					}
		...
		},
		{
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					if len(args) < 1 {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, "Expect at least 1 argument.")
					}
