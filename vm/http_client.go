	...
	return []*BuiltinMethodObject{
		{
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					if len(args) != 1 {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, errors.WrongNumberOfArgumentFormat, 1, len(args))
					}

					u, ok := args[0].(*StringObject)
					if !ok {
						return t.vm.initErrorObject(errors.TypeError, instruction.sourceLine, errors.WrongArgumentTypeFormat, classes.StringClass, u.Class().Name)
					}
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					...
					if args[0].Class().Name != httpRequestClass.Name {
						return t.vm.initErrorObject(errors.TypeError, instruction.sourceLine, errors.WrongArgumentTypeFormat, "HTTP Response", args[0].Class().Name)
					}

					goReq, err := requestGobyToGo(args[0])
					if err != nil {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, err.Error())
					}
