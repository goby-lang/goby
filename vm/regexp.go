	...
	return []*BuiltinMethodObject{
		{
			...
			Fn: func(receiver Object, instruction *instruction) builtinMethodBody {
				return func(t *thread, args []Object, blockFrame *normalCallFrame) Object {
					...
					if !ok {
						return t.vm.initErrorObject(errors.TypeError, instruction.sourceLine, errors.WrongArgumentTypeFormat, classes.StringClass, arg.Class().Name)
					}

					r := t.vm.initRegexpObject(args[0].toString())
					if r == nil {
						return t.vm.initErrorObject(errors.ArgumentError, instruction.sourceLine, "Invalid regexp: %v", args[0].toString())
					}
					return r
				}
			},
